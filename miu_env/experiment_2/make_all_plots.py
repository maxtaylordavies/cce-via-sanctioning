...
DATA_DIR = Path("data/miu_env/experiment_2")
OUTPUT_DIR = Path("figures/miu_env/experiment_2")
...
for run_type in raw_data.keys():
    ...
    raw_data[run_type]["payoff"] = outputs["payoffs"]
    raw_data[run_type]["avg_level"] = outputs["avg_levels"]
    raw_data[run_type]["max_level"] = outputs["max_levels"]
...
for run_type, data in raw_data.items():
    ...
    data["sampled_payoff"] = data["payoff"][:, ::CGS_INTERVAL]
    data["sampled_avg_level"] = data["avg_level"][:, ::CGS_INTERVAL]
    data["sampled_max_level"] = data["max_level"][:, ::CGS_INTERVAL]
...
for seed_idx in range(max_n_seeds):
    for run_type in ["real", "neutral"]:
        ...
        processed_data[run_type]["population_average_payoff"].append(
            raw_data[run_type]["sampled_payoff"][seed_idx]
        )
        processed_data[run_type]["population_average_avg_level"].append(
            raw_data[run_type]["sampled_avg_level"][seed_idx]
        )
        processed_data[run_type]["population_average_max_level"].append(
            raw_data[run_type]["sampled_max_level"][seed_idx]
        )
...
final_window_len = max(
    1, processed_data_np["real"]["population_average_payoff"].shape[1] // 10
)
for run_type in ["real", "neutral"]:
