...
_save_fig = lambda fig, name: save_fig(fig, name, subfolder="experiment_2")


def get_data_dir(config, variant):
    return Path(f"data/{config.name}/experiment_2/{variant}")


def load_reference_gains(path=REFERENCE_GAINS_PATH):
    if not path.exists():
        raise FileNotFoundError(
            f"Reference gain file not found at {path}. Run "
            "plot_experiment_1_data.py first to generate it."
        )
    ...
    with path.open(newline="") as f:
        ...
        for row in reader:
            ...
            reference_gains[key] = float(row["reference_prestige_gain"])
...
def get_p_innovate_array(outputs):
    ...
    if "agent_roles" in outputs:
        roles = np.asarray(outputs["agent_roles"])
        return (roles == role_innovate).mean(axis=-1)
    if "p_innovate" in outputs:
        return np.asarray(outputs["p_innovate"])
    return None
...
def process_environment(config, variant):
    print(f"Processing {config.name} ({variant})...")
    data_dir = get_data_dir(config, variant)
    raw = {}
    ...
    if config.name == "mesoudi_env":
        max_t = min(max_t, MESOUDI_PLOT_T_MAX)
    ...
    for run_type, data in raw.items():
        ...
        if config.name == "recipe_world_env" and variant == "gift":
            sampled_norm_values = -sampled_norm_values
    ...
    return raw, processed, sampled_timesteps
...
def build_processed_cache():
    ...
    for variant in VARIANTS:
        for config in ENVIRONMENTS:
            _, processed, ts = process_environment(config, variant)
            cache[(variant, config.name)] = {
                "processed": processed,
                "timesteps": ts,
                "reference_gain": reference_gains[(config.name, variant)],
            }
    return cache
...
def plot_main_figure(processed_cache, norm_window="all"):
    ...
    for row_idx, variant in enumerate(VARIANTS):
        for col_idx, env_config in enumerate(ENVIRONMENTS):
            panel_data = processed_cache[(variant, env_config.name)]
            xlabel_detail = (
    ...
    for row_idx, variant in enumerate(VARIANTS):
        for col_idx, env_config in enumerate(ENVIRONMENTS):
            ...
            if metric == "norm":
                values_by_run = {
                    run_type: (
                        processed[run_type]["average_norm_values"]
                        / panel_data["reference_gain"]
                    )
                    for run_type in RUN_TYPES
                }
                ...
                values_by_run = {
                    run_type: processed[run_type]["score"] for run_type in RUN_TYPES
                }
...
def plot_lineage_figures(configs, variant):
    reference_gains = load_reference_gains()
    for config in configs:
        raw, _, _ = process_environment(config, variant)
        real = raw["real"]
        reference_gain = reference_gains[(config.name, variant)]
        ...
        for seed_idx in range(n_lineage_seeds):
            instance_norm_series = (
                build_group_instance_norm_series(
            ...
            axs[seed_idx].set(
                ...
                xlabel="$t$",
                ylabel="$g/g_{0.5}$" if seed_idx == 0 else None,
            )
            sns.despine(ax=axs[seed_idx], left=True, bottom=True)

        fig.suptitle(f"{config.title}: group lineages", y=1.02, fontweight="bold")
...
def main():
    ...
    # for variant in VARIANTS:
    #     plot_lineage_figures(ENVIRONMENTS, variant)
