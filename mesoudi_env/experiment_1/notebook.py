...
@jax.jit
def distribute_payoffs(all_traits, all_prestiges, b, prestige_baseline, pool_share):
    individual_payoffs = all_traits.sum(axis=1) * b
    total = individual_payoffs.sum()
    prestige_weights = prestige_baseline + jnp.maximum(all_prestiges, 0.0)
    weights = prestige_weights / prestige_weights.sum()
    return ((1.0 - pool_share) * individual_payoffs) + (pool_share * total * weights)
...
def run_simulation_loop(
    ...
    def body_fn(carry, t):
        ...
        curr_payoffs = distribute_payoffs(
            all_traits, all_prestiges, b, prestige_baseline, pool_share
        )

        # update traits + prestiges and compute new payoffs
        new_all_traits, n_innovated, n_imitated = update_traits(
            update_key, all_traits, all_roles, neighbours_mask, curr_L, p_i, p_c
        )
        prestige_grants = prestige_gain * n_innovated.astype(jnp.float32)
        # prestige_grants = prestige_gain * (all_roles == ROLE_INNOVATE).astype(jnp.float32)
        new_all_prestiges = all_prestiges + prestige_grants

        new_payoffs = distribute_payoffs(
            new_all_traits, new_all_prestiges, b, prestige_baseline, pool_share
        )
...
def main():
    ...
    prestige_gain_vals = jnp.linspace(0.0, 0.01, 21)
    prestige_decay = 0.01
    prestige_baseline = 1.0
    pool_share = 0.5
    ...
    def run_with_prestige_gain(key, prestige_gain):
        return jax.block_until_ready(
            run_simulation_loop(
                ...
                prestige_baseline=prestige_baseline,
    ...
    simulation_outputs = {
        ...
        "prestige_baseline": np.float32(prestige_baseline),
        "pool_share": np.float32(pool_share),
